...
void LRU(int pageTableSize, int* pageRequests, int numPageRequests) {
    ...
    int* pageFaultSequence = (int*)malloc(numPageRequests * sizeof(int)); //page fault sequence //
    int pageFaults = 0;
    int currentTime = 0;
    ...
    for (int i = 0; i < numPageRequests; i++) {
        ...
        if (page_index != -1) {
            ...
            //  page fault page number //
            pageFaultSequence[pageFaults] = page_number;
            pageFaults++;
